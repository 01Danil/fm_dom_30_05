...
function createCard(actor) {
  const h2 = createElement("h2",{ classNames: ["card-fullname"] }, document.createTextNode(actor.name || "Noname")
  );
  const p = createElement("p",{ classNames: ["card-description"] },document.createTextNode(actor.birthdate || "Month d, YYYY")
  );
  const article = createElement("article",{ classNames: ["card-container"] },createWrapper(actor),h2,p
  );
